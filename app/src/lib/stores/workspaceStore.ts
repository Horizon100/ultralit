...
function createWorkspaceStore() {
  const initialWorkspaces = browser ? JSON.parse(localStorage.getItem('userWorkspaces') || '[]') : [];
  const initialCurrentWorkspaceId = browser ? localStorage.getItem('currentWorkspaceId') || null : null;
  
  const { subscribe, set, update } = writable<{workspaces: Workspaces[], currentWorkspaceId: string | null, updateStatus: string}>({
  ...
  return {
    ...
    loadWorkspaces: async (userId: string): Promise<Workspaces[]> => {
      try {
        ...
        update(state => ({
          ...state,
          workspaces,
          currentWorkspaceId: state.currentWorkspaceId || (workspaces.length > 0 ? workspaces[0].id : null),
        ...
        set({ workspaces: [], currentWorkspaceId: null, updateStatus: 'Failed to load workspaces' });
        setTimeout(() => update(state => ({ ...state, updateStatus: '' })), 3000);
        return [];
      }
    },
    addWorkspace: (workspace: Workspaces) => update(state => ({
      ...state,
    ...
    removeWorkspace: (id: string) => update(state => ({
      ...state,
      workspaces: state.workspaces.filter(w => w.id !== id),
      updateStatus: 'Workspace removed successfully'
    })),
    setCurrentWorkspace: (id: string) => update(state => ({
      ...state,
