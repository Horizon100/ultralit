	...
	const store = {
		...
		updatePreferences: async (email: string, preferences: NewsletterPreferences) => {
			try {
				...
				const updatedSubscription = await pb
					.collection('subscriptions')
					.update<NewsletterSubscription>(subscriptions[0].id, {
						newsletter: preferences.newsletter,
						events: preferences.events
					});

				// Update local store
				update((subs) => subs.map((sub) => (sub.email === email ? updatedSubscription : sub)));
		...
		verifySubscription: async (email: string, token: string) => {
			try {
				...
				const verifiedSubscription = await pb
					.collection('subscriptions')
					.update<NewsletterSubscription>(subscriptions[0].id, {
						verified: true
					});
				...
				// Update local store
				update((subs) => subs.map((sub) => (sub.email === email ? verifiedSubscription : sub)));
