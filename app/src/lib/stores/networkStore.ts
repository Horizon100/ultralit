...
function createNetworkStore() {
	...
	return {
		subscribe,
		setRootAgent: (agent: AIAgent) =>
			update((state) => ({
				...state,
				rootAgent: agent
			})),
		addChildAgent: (agent: AIAgent) =>
			update((state) => ({
				...state,
				childAgents: [...state.childAgents, agent]
			})),
		addChildAgents: (agents: AIAgent[]) =>
			update((state) => ({
				...state,
		...
		updateAgent: (id: string, updates: Partial<AIAgent>) =>
			update((state) => ({
				...state,
				rootAgent:
					state.rootAgent?.id === id ? { ...state.rootAgent, ...updates } : state.rootAgent,
				childAgents: state.childAgents.map((agent) =>
					agent.id === id ? { ...agent, ...updates } : agent
				)
			})),
		removeChildAgent: (id: string) =>
			update((state) => ({
				...state,
