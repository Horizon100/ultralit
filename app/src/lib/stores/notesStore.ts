	...
	return {
		...
		deleteFolder: async (id: string) => {
			...
			update((store) => ({
				...
				notes: Object.fromEntries(Object.entries(store.notes).filter(([key]) => key !== id))
			}));
		},
		...
		updateNote: async (id: string, noteData: Partial<Notes>) => {
			...
			update((store) => ({
				...
				notes: {
					...store.notes,
					[updatedNote.folder]: (store.notes[updatedNote.folder] || []).map((n) =>
						n.id === id ? { ...n, ...updatedNote } : n
					)
				},
				openTabs: store.openTabs.map((n) => (n.id === id ? { ...n, ...updatedNote } : n)),
				currentNote:
					store.currentNote?.id === id
						? { ...store.currentNote, ...updatedNote }
						: store.currentNote
			}));
		},
		...
		setCurrentFolder: (folder: Folders | null) =>
			update((store) => ({ ...store, currentFolder: folder })),
