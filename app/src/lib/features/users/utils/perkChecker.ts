...
export async function checkEligiblePerks(
	stats: UserStats
): Promise<Omit<Perk, 'id' | 'created' | 'updated' | 'achievedBy'>[]> {
	try {
		...
		return PERKS.filter((perk) => {
			return perk.filterConditions.every((condition: PerkCondition) => {
				...
				switch (condition.parameter) {
					case 'messages':
						userValue = stats.messageCount;
						break;
					case 'threads':
						userValue = stats.threadCount;
						break;
					case 'tasks':
						userValue = stats.taskCount;
						break;
					case 'tags':
						userValue = stats.tagCount;
						break;
				}
...
export async function updateUserPerks(
	userId: string,
	eligiblePerks: Omit<Perk, 'id' | 'created' | 'updated' | 'achievedBy'>[]
): Promise<string[]> {
	try {
		...
		const eligiblePerkIds = eligiblePerks
			.map((eligiblePerk) => {
