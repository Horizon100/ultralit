...
<script lang="ts">
  ...
  function createBuildingLayout(rooms: GameRoom[]): BuildingCell[][] {
    const layout: BuildingCell[][] = Array(BUILDING_GRID_SIZE).fill(null).map(() =>
      Array(BUILDING_GRID_SIZE).fill(null).map(() => ({ type: 'empty', room: null }))
    );
  ...
  function getRoomColor(roomType: string) {
    const colors = {
      hr: '#3b82f6', library: '#8b5cf6', manufacturing: '#f59e0b', assembly: '#ef4444',
      inbound: '#10b981', outbound: '#06b6d4', helpdesk: '#f97316', training: '#84cc16',
      conference: '#ec4899', lobby: '#6366f1', cafeteria: '#f59e0b', office: '#3b82f6'
    };
  ...
  function getRoomIcon(roomType: string) {
    const icons = {
      hr: '👥', library: '📚', manufacturing: '⚙️', assembly: '🔧',
      inbound: '📥', outbound: '📤', helpdesk: '🎧', training: '🎓',
      conference: '🏛️', lobby: '🏢', cafeteria: '🍽️', office: '💼'
    };
  ...
  >
    ...
    <div class="building-interior">
      ...
      <div class="building-grid">
        ...
        <div 
          class="hero-avatar"
          style="left: {heroPosition.x}px; top: {heroPosition.y}px;"
        >
...
<style>
  ...
  .room-content, .entrance-content, .lobby-content {
    text-align: center;
    color: white;
    padding: 2px;
  }
  
  .room-icon, .entrance-icon, .lobby-icon {
    font-size: 14px;
    line-height: 1;
  }
