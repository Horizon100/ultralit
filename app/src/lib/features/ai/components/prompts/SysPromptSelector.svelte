<script lang="ts">
	...
	function applySystemPrompt(promptText: string, promptName?: string) {
		...
		const promptType =
			promptName ||
			Object.entries(SYSTEM_PROMPTS).find(([key, value]) => value === promptText)?.[0] ||
			'NORMAL';
	...
	<div class="prompt-wrapper">
		...
		<div class="prompt-list system-prompts">
			{#each Object.entries(SYSTEM_PROMPTS) as [name, text]}
				<div
					class="prompt-item system-prompt-item"
					on:click={() => applySystemPrompt(text, name)}
					class:selected={dualComparisonMode
						? selectedSystemPrompts.includes(text)
					...
					<div class="button-container">
						{#if dualComparisonMode}
							<button
								...
								disabled={!selectedSystemPrompts.includes(text) &&
									selectedSystemPrompts.length >= 2}
							>
