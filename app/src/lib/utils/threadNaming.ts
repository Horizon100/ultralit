...
export async function generateThreadName(
    ...
    try {
        ...
        const prompt = {
            role: 'system',
            content: `Create a concise, descriptive title (max 50 chars) for a conversation starting with: User: ${userMessage} Assistant: ${aiResponse} Return only the title, no quotes or explanation.`
        };
...
export async function shouldUpdateThreadName(messages: Messages[]): Promise<boolean> {
    try {
        ...
        console.error('Error checking thread name update:', error);
        if (error instanceof Error && error.message === 'User is not authenticated') {
            throw error;
        }
        return false;
...
export async function updateThreadNameIfNeeded(
    ...
    try {
        ...
        if (error instanceof Error && error.message === 'User is not authenticated') {
            throw new Error('Authentication required to update thread name');
        }
    } finally {
