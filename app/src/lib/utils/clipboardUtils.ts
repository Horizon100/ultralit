...
export function getClipboardSupport() {
	...
	const fallbackSupport = !!(document.queryCommandSupported && document.queryCommandSupported('copy'));
	
	return {
		modern: modernSupport,
		fallback: fallbackSupported,
		supported: modernSupport || fallbackSupport,
		secureContext: window.isSecureContext,
		protocol: window.location.protocol,
		userAgent: navigator.userAgent.includes('iPhone') ? 'iOS' : 
