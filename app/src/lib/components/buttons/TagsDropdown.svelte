...
<script lang="ts">
  ...
  import { Tag, Search, ChevronDown, Plus, Check, Trash2, ChevronLeft, X, Filter } from 'lucide-svelte';
  ...
  $: displayText = selectedTagNames.length > 0 
  ...
  async function handleDeleteTag(e: Event, tagId: string) {
    ...
    try {
      ...
      const confirmed = confirm('Are you sure you want to delete this tag? This action cannot be undone.');
      if (!confirmed) return;
...
.create-btn {
  ...
  .create-btn:disabled, .clear-btn:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }
