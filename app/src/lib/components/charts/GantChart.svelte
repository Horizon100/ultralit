<script lang="ts">
	...
	function handleTaskDragMove(event: MouseEvent) {
		...
		const pixelsPerDay =
			document.querySelector('.gantt-timeline')?.clientWidth ||
			1000 / getDaysBetween(get(dateRange).start, get(dateRange).end);
	...
	function generateGridRows(): GridRow[] {
		...
		const parentTasks = allTasks.filter(
			(task) => !task.parent_task && getTaskDependencies(task.id).length > 0
		);
...
<div class="gantt-container">
	...
	{:else}
				...
				<div class="gantt-chart-grid">
					...
					{#each generateGridRows() as row, rowIndex}
						<div
							...
							class:parent-row={row.isParentRow}
						>
		...
		{#if showTaskInfo && selectedTask}
			<div class="task-info-modal" transition:fade={{ duration: 150 }} on:click={closeTaskInfo}>
				<div
					...
					<div class="task-info-body">
						<div class="info-section">
							...
							<span>
								<div
									class="status-badge"
									style="background-color: {statusColors[selectedTask.status]}"
								>
						...
						<div class="info-section">
							...
							<div class="timeline-info">
								...
								<div>
									<strong>Due:</strong>
									{selectedTask.due_date ? selectedTask.due_date.toLocaleDateString() : 'Not set'}
								</div>
								{#if selectedTask.start_date && selectedTask.due_date}
									<div>
										<strong>Duration:</strong>
										{getDaysBetween(selectedTask.start_date, selectedTask.due_date)} days
									</div>
								{/if}
							</div>
						</div>
