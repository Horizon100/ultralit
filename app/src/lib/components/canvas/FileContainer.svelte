<script lang="ts">
  ...
  import { FileText, Image, FileSpreadsheet, Headphones, Video, Maximize2, Minimize2, Link, Code, Presentation } from 'lucide-svelte';
...
////////////////////////////////////////////////////
  ...
  function getFileIcon(fileType: string) {
    ...
    if (fileType === 'text/plain' || fileType === 'application/json' || fileType === 'text/javascript' || fileType === 'text/csv') return Code;
  ...
  function getGradientBorder(fileType: string) {
    ...
    if (fileType.includes('sheet') || fileType.includes('excel') || fileType.includes('csv')) return 'linear-gradient(to right, #FFC107, #FFEB3B)';
    ...
    if (fileType.includes('presentation') || fileType.includes('powerpoint')) return 'linear-gradient(to right, #FF9800, #FFC107)';
  ...
  async function loadFileContent() {
    if (file instanceof File) {
        if (fileType.startsWith('image/') || fileType.startsWith('video/') || fileType.startsWith('audio/')) {
            ...
            fileContent = "PDF preview is not available. Please download the file to view its contents.";
...
<div
  ...
  style="left: {x * transform.scale + transform.offsetX}px; top: {y * transform.scale + transform.offsetY}px; transform: scale({transform.scale});"
...
>
  <div class="file-header">
    ...
    <button class="expand-button" on:click={() => { isExpanded = !isExpanded; loadFileContent(); }}>
  ...
  {#if isExpanded && isClient}
    <div class="file-content" transition:scale>
      ...
      {:else if fileType === 'text/plain' || fileType === 'application/json' || fileType === 'text/javascript'}
        <pre><code>{@html highlightCode(fileContent, fileType === 'application/json' ? 'json' : 'javascript')}</code></pre>
