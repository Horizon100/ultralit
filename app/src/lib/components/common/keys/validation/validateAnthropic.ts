...
export async function anthropic(key: string): Promise<{ isValid: boolean, errorMessage?: string }> {
  try {
    ...
    if (err.message.includes("Your credit balance is too low")) {
      return { isValid: false, errorMessage: "Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits." };
    }
