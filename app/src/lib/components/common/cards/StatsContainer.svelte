<script lang="ts">
	...
	import { PERKS, INCREMENTS } from '$lib/data/perks'; 
		...
		const eligiblePerks = PERKS.filter(perk => {
			const isEligible = perk.filterConditions.every(condition => {
				...
				switch (condition.parameter) {
					case 'messages': userValue = messageCount; break;
					case 'threads': userValue = threadCount; break;
					case 'tasks': userValue = taskCount; break;
					case 'tags': userValue = tagCount; break;
				}
				
				// Evaluate the condition
				let result = false;
				switch (condition.operator) {
					case '=': result = userValue === condition.value; break;
					case '>': result = userValue > condition.value; break;
					case '>=': result = userValue >= condition.value; break;
					case '<': result = userValue < condition.value; break;
					case '<=': result = userValue <= condition.value; break;
				...
				console.log(`DEBUG: Condition check - ${condition.parameter} ${condition.operator} ${condition.value}, User value: ${userValue}, Result: ${result}`);
				return result;
			});
	...
	function getEligiblePerksLocally(): Perk[] {
		console.log('Getting eligible perks locally with stats:', { threadCount, messageCount, taskCount, tagCount });
...
function getNextPerkTarget(statValue: number, statType: 'messages' | 'threads' | 'tasks' | 'tags'): { value: number; perk: Perk | null; progress: number } {
    ...
    const targetPerk = PERKS.find(perk => {
      return perk.filterConditions.length === 1 && 
  ...
  $: timerTarget = 3600;
	onMount(() => {
		...
		if (threadCount === 0 && messageCount === 0 && taskCount === 0 && tagCount === 0 && timerCount === 0) {
...
<div class="stats-container" transition:fade={{ duration: 200 }}>
  ...
  <div class="stat-item" style="--progress: {calculatePercentage(timerCount, timerTarget)}%">
    ...
    <div class="target">
	  ...
	  <div class="perks-summary">
		<button class="perks-button" on:click={togglePerksPanel} class:checking={checkingPerks}>
			{checkingPerks ? 'Checking...' : `${perks.length} ${$t('dashboard.perks', 'Perks')} ${showPerksPanel ? '▲' : '▼'}`}
		</button>
	...
	<div class="perks-panel" transition:fade={{ duration: 200 }}>
		{#if perks.length === 0}
		<div class="no-perks">
			{checkingPerks ? 'Checking for perks...' : 'No perks unlocked yet. Keep using the platform!'}
		</div>
		{:else}
...
<style lang="scss">
	...
	* {
	...
	@keyframes highlight {
		0% { background: rgba(1, 149, 137, 0.1); }
		50% { background: rgba(1, 149, 137, 0.3); }
		100% { background: rgba(1, 149, 137, 0.1); }
	}
