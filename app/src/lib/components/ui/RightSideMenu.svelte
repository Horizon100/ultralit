...
<div class="side-menu" style="width: {width}px;" on:mouseleave={() => dispatch('mouseleave')} role="complementary">
  <div class="tabs">
    <button class:active={activeTab === 'assets'} on:click={() => setActiveTab('assets')}>Assets</button>

    <button class:active={activeTab === 'assistant'} on:click={() => setActiveTab('assistant')}>Assistant</button>
  ...
  <div class="content">
    {#if activeTab === 'assets'}
      <div in:slideTransition={{ direction: -1 }} out:slideTransition={{ direction: 1 }}>
        <Assets on:addShape={handleAddShape} />
      </div>
...
<style>
  .side-menu {
    ...
    height: 100%;
    background-color: rgba(54, 63, 63, 0.10);
