...
<div class="side-menu" style="width: {width}px;" on:mouseleave={() => dispatch('mouseleave')}>
  <div class="tabs">
    <button class:active={activeTab === 'layers'} on:click={() => setActiveTab('layers')}>Layers</button>
    <button class:active={activeTab === 'style'} on:click={() => setActiveTab('style')}>Style</button>
  </div>
  <div class="content">
    {#if activeTab === 'layers'}
      {#if selectedShape && editedAgent}
        ...
        <div class="field">
          ...
          <input id="name" type="text" bind:value={editedAgent.name}   on:input={(e) => console.log('Input changed:', e.target.value)}
          />
