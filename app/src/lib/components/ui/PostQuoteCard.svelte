<script lang="ts">
  ...
  async function fetchQuotedPost() {
    ...
    try {
      ...
      // Find the quoted post in the response
      quotedPost = allPosts.find((p: PostWithInteractions) => p.id === post.quotedPost);
      
      if (!quotedPost) {
        // If not found in recent posts, try to get it from store
        const storePosts = $postStore.posts;
        quotedPost = storePosts.find(p => p.id === post.quotedPost) || null;
      }
    } catch (error) {
  ...
  function handleInteraction(event: CustomEvent<{ postId: string; action: 'upvote' | 'repost' | 'read' | 'share' }>) {
  ...
  function handleQuote(event: CustomEvent<{ content: string; attachments: File[]; quotedPostId: string }>) {
...
<div class="quote-card">
  ...
  <div class="quote-header">
    <Quote size={14} />
    <span>Quoted by {quotedBy.name || quotedBy.username || 'Unknown User'}</span>
  </div>
