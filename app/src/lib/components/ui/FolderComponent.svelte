...
<div class="folder" 
    ...
    <div class="folder-content" transition:slide>
        {#each notes as note (note.id)}
            <div class="note"
                 draggable="true"
                 on:dragstart={(e) => dispatchFolderEvent('handleDragStart', { event: e, item: note, isFolder: false })}
                 on:click={() => openNote(note)}
                 on:contextmenu|preventDefault={(e) => dispatchFolderEvent('showContextMenu', { event: e, item: note, isFolder: false })}
