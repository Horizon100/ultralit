<script lang="ts">
    ...
    import { Plus, Trash2, Bot, Settings, CopyPlus, Search, ArrowUpDown, Upload, Filter, Activity, Compass, ServerCog, OctagonPause, AlertCircle, RefreshCcw, Cpu, HeadphonesIcon, ShieldCheck, ArrowLeft, CircleOff } from 'lucide-svelte';
	...
	import NodeCreationModal from '../node/NodeCreationModal.svelte';
    ...
    function updateAppliedFiltersCount() {
        appliedFiltersCount = (selectedRole ? 1 : 0) + 
                            (selectedStatus ? 1 : 0) + 
                            selectedTags.length;
    }
    ...
    .filter(agent => 
        ...
        (selectedTags.length === 0 || (agent.tags && selectedTags.every(tag => agent.tags.includes(tag))))
    )
    .sort((a, b) => {
            ...
            {#each filteredAgents as agent (agent.id)}
                <div class="agent-item" on:click={() => showGenerator(agent)}>
                    ...
                    <div class="action-buttons" transition:fade={{ duration: 200 }}>
                        ...
                        <button class="control-button-delete" on:click|stopPropagation={() => handleDelete(agent)}>
                            <Trash2 size={24}/>
                        </button>
                    </div>
                </div>
            ...
            <div class="search-and-sort-container">
                ...
                <div class="options">
                    <button class="filter-toggle-button" on:click={toggleFilters} class:active={showFilters}>
                        {showFilters ? 'Hide' : `Filters${appliedFiltersCount > 0 ? ` (${appliedFiltersCount})` : ''}`}
                    </button>                    
                    ...
                    <div class="filter-container">
                        <div class="filter-group">
                            <div class="filter-row">
                                ...
                                <button class="filter-button" class:active={!selectedRole} on:click={() => selectRole(null)}>
                                    <CircleOff/>
                                </button>
                            </div>
                            <div class="filter-row">
                                ...
                                {#each roles as role}
                                    <button class="filter-button" class:active={selectedRole === role} on:click={() => selectRole(role)}>
                        ...
                        <div class="filter-group">
                            <div class="filter-row">
                                ...
                                <button class="filter-button" class:active={!selectedStatus} on:click={() => selectedStatus = null}>
                                    <CircleOff/>
                                </button>
                            </div>
                            <div class="filter-row">
                                {#each statuses as status}
                                    <button class="filter-button status-{status}" class:active={selectedStatus === status} on:click={() => selectStatus(status)}>
                        ...
                        <div class="filter-group">
                            <div class="tag-filter">
                                {#each [...new Set(agents.flatMap(agent => agent.tags || []).filter(Boolean))] as tag}
                                    <button class="tag-button" class:active={selectedTags.includes(tag)} on:click={() => toggleTag(tag)}>
                                        {tag}
                                    </button>
                                {/each}
                            </div>
        ...
        <div class="column form-column" transition:fly={{ x: 300, duration: 300 }}>
            <div class="form-content">
                ...
                <div class="form-group">
                    ...
                    <textarea id="agentPrompt" bind:value={agentPrompt} placeholder="Enter agent prompt..."></textarea>
                </div>
                <hr>
                <div class="form-group">
                    <label for="agentName">NAME</label>
                    <input type="text" id="agentName" bind:value={agentName} placeholder="Enter agent name...">
                </div>

                <div class="form-group">
                    <label for="agentDescription">DESCRIPTION</label>
                    <textarea id="agentDescription" bind:value={agentDescription} placeholder="Enter agent description..."></textarea>
                </div>


                <div class="form-group">
                    <label for="agentMaxAttempts">MAX ATTEMPTS {agentMaxAttempts}</label>
                    <input type="range" id="agentMaxAttempts" bind:value={agentMaxAttempts} min={MIN_ATTEMPTS} max={MAX_ATTEMPTS}>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        {#each availableModels as model (model.id)}
                            <label>
                                <input type="checkbox" checked={agentModel.includes(model.id)} on:change={() => toggleModel(model.id)}>
                                {model.name}
                            </label>
                        {/each}
                    </div>
...
<style>
    ...
    .generator-content {
        flex-grow: 1;
        overflow: hidden;
        background: radial-gradient(circle at top, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0) 90%);
        padding: 20px;
        border-radius: 50px;
        justify-content: flex-start;
        height: 80vh;
    ...
    textarea {
        ...
        border:none;
        font-size: 20px;
        background: radial-gradient(circle at left, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0) 90%);
        resize:none;
    ...
    .status-active { background-color: #1f1f1f; color: white; }
    .status-inactive { background-color: #1f1f1f; color: white; }
    .status-maintenance { background-color: #1f1f1f; color: black; }
    .status-paused { background-color: #1f1f1f; color: white; }

    .container-row:hover .data-counts { display: none; }
    .container-row:hover .action-buttons { display: flex; }

    .data-counts, .action-buttons {
        position: absolute;
        right: 2px;
        bottom: 2px;
    ...
    .control-button, .control-button-delete {
        background-color: transparent;
        color: gray;
        transition: all 0.3s ease;
    }
   ...
   .sort-container {
    ...
    .filter-button:hover, .tag-button:hover {
        background-color: #616161;
    }

    .filter-button.active, .tag-button.active {
        background-color: #1f1f1f;
        color: white;
    }
