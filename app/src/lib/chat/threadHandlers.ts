...
export const showThreadList = derived(threadsStore, ($store) => $store.showThreadList);
	...
	if (currentThreadId && editedThreadName.trim() !== '') {
		try {
			...
			threads = threads.map((thread) =>
				thread.id === currentThreadId ? { ...thread, name: editedThreadName.trim() } : thread
			);
...
export function toggleDateExpansion(date: string, event?: Event) {
	...
	if (newExpandedDates.has(date)) {
		if (date !== groupedMessages[0]?.date) {
			// If it's not the most recent group
	...
	return Object.entries(groups)
		...
		.sort((a, b) => {
			...
			if (groupPriority(a.group) === 2 && groupPriority(b.group) === 2) {
				return new Date(b.threads[0].updated).getTime() - new Date(a.threads[0].updated).getTime();
			}
			return 0;
		});
}
