...
class GameClient {
  ...
  private async syncPositionToServer(userId: string) {
    ...
    if (this.lastSentPosition && !this.shouldSyncPosition(this.lastSentPosition, this.pendingPosition)) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(`[GAME CLIENT] Failed to sync position: ${response.status}`);
