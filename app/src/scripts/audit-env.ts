      ...
      if (file.isDirectory() && file.name !== 'node_modules' && file.name !== '.svelte-kit') {
        ...
        if (content.includes('$env/static/public') || content.includes('$env/static/private')) {
          const importMatches = [
            ...
            ...content.matchAll(/import\s+{\s*([^}]+)\s*}\s+from\s+['"]?\$env\/static\/(?:public|private)['"]?/g)
          ];
...
function auditEnvVars() {
  ...
  // If no issues found
  if (undefinedVars.length === 0 && 
      (!definedVars['.env.development'] || !definedVars['.env.production'] || 
       !definedVars['.env.development'].filter(v => !definedVars['.env.production'].includes(v)).length)) {
    console.log('\nNo issues found!');
  }
}
