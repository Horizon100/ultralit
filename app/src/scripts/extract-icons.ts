	...
	private extractIconsFromFile(content: string): void {
		...
		while ((match = importRegex.exec(content)) !== null) {
			...
			iconNames.forEach((icon: string) => {
				...
				const cleanIcon = icon.replace(/\s+as\s+\w+/g, '').trim();
	...
	async downloadIcons(): Promise<void> {
		...
		const downloadPromises = Array.from(this.usedIcons).map(async (iconName: string) => {
			...
			try {
				...
				console.error(
					`❌ Failed to download ${iconName} (${kebabCase}):`,
				...
				if (!downloaded) {
					console.error(
						`❌ Could not find icon for: ${iconName}. Tried: ${kebabCase}, ${alternatives.join(', ')}`
					);
