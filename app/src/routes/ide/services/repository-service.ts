...
// Folder operations
export async function getFolders(repositoryId: string, branch?: string, parentId?: string): Promise<CodeFolders[]> {
    try {
        const params = new URLSearchParams({
            repositoryId
        });
...
// File operations
export async function getFiles(repositoryId: string, branch?: string, path?: string): Promise<CodeFiles[]> {
    try {
        const params = new URLSearchParams({
            repositoryId
        });
...
export async function createBranch(repositoryId: string, name: string, sourceBranch: string): Promise<any> {
    try {
        const response = await fetch(`/api/repositories/${repositoryId}/branches`, {
            method: 'POST',
            headers: {
