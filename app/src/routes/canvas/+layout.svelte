<script lang="ts">
    ...
    let connectors: Array<{ id: string; startId: string; endId: string; startPoint: string; endPoint: string }> = [];
    ...
    onMount(() => {
        ...
        const unsubscribe = agentStore.subscribe((value: { agents: AIAgent[]; updateStatus: string }) => {
    ...
    function getNearestConnectionPoint(shape: Shape, x: number, y: number) {
        ...
        points.forEach(point => {
            ...
            const distance = Math.sqrt(Math.pow(x - connectionPoint.x, 2) + Math.pow(y - connectionPoint.y, 2));
        ...
        <div class="svg-wrapper" style="flex: 1; display: flex;">
            ...
            {#each shapes as shape (shape.id)}
                <div 
                    ...
                    style="
                        position: absolute;
                        left: calc({((shape.x ?? 0) * scale) + $offsetX}px + {$leftSideMenuWidth}px - {$rightSideMenuWidth}px);
                    ...
                    {#if showConnectionPoints}
                        <button class="connection-point top" on:mousedown={() => startConnection('top', shape.id)}></button>
                        <button class="connection-point right" on:mousedown={() => startConnection('right', shape.id)}></button>
                        <button class="connection-point bottom" on:mousedown={() => startConnection('bottom', shape.id)}></button>
                        <button class="connection-point left" on:mousedown={() => startConnection('left', shape.id)}></button>
                    {/if}
                </div>
                {/each}
            </div>
