...
export const GET: RequestHandler = async ({ params, locals }) =>
  apiTryCatch(async () => {
    ...
    const messageResult = await pbTryCatch(pb.collection('messages').getOne(messageId), 'fetch message');
    const message = unwrap(messageResult);

    if (message.thread) {
      try {
        const threadResult = await pbTryCatch(pb.collection('threads').getOne(message.thread), 'fetch thread');
  ...
  apiTryCatch(async () => {
    ...
    if (existingMessage.user !== currentUserId) {
      ...
      if (existingMessage.thread) {
        try {
          const threadResult = await pbTryCatch(pb.collection('threads').getOne(existingMessage.thread), 'fetch thread');
          const thread = unwrap(threadResult);
    ...
    const updateResult = await pbTryCatch(pb.collection('messages').update(messageId, data), 'update message');
