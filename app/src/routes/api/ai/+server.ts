...
export const POST: RequestHandler = async (event) =>
	apiTryCatch(async () => {
		...
		const userDataResult = await pbTryCatch(
			pbServer.pb.collection('users').getOne(user.id),
			'fetch user data'
		);
		...
		if (userData.api_keys) {
			try {
				...
				throw new Error(
					'Error decrypting API keys: ' + (e instanceof Error ? e.message : String(e))
				);
			}
		} else {
		...
		if (!apiKey) {
			...
			throw new Error(
				`${model.provider} API key not configured. Available providers: ${availableProviders.join(', ')}`
			);
		}

		console.log(
			'🔍 AI API Debug - Using API key for provider:',
			model.provider,
			'Key starts with:',
		...
		try {
			if (
				...
				const modelName =
					model.api_type || (model.provider === 'openai' ? 'gpt-3.5-turbo' : 'deepseek-chat');
				...
				console.log(
					'🔍 AI API Debug - Completion received, choices count:',
			...
			throw new Error(
				`AI provider error: ${aiError instanceof Error ? aiError.message : String(aiError)}`
			);
		}
