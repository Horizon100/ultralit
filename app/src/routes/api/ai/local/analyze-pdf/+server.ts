...
Format your response with clear sections and bullet points for readability.`,
			model,
			system:
				'You are a document analysis expert. Provide clear, structured, and actionable insights from documents. Focus on extracting valuable information and practical takeaways.',
			temperature: 0.2,
			max_tokens: 1000,
			auto_optimize: false
		})
...
function getDocumentType(text: string): string {
	...
	const patterns = [
		...
		{
			keywords: ['report', 'analysis', 'findings', 'summary', 'recommendations'],
			type: 'Business Report'
		},
		{
			keywords: ['specification', 'requirements', 'technical', 'design', 'implementation'],
			type: 'Technical Specification'
		},
		{
			keywords: ['policy', 'guidelines', 'procedures', 'rules', 'compliance'],
			type: 'Policy Document'
		},
		{
			keywords: ['proposal', 'project', 'timeline', 'budget', 'deliverables'],
