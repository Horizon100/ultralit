...
import { apiTryCatch, pbTryCatch, isFailure } from '$lib/utils/errorUtils';
	...
	return await apiTryCatch(
		async () => {
			...
			const children = Object.entries(statusGroups).map(([status, statusTasks]) => {
				...
				const priorityChildren = Object.entries(priorityGroups).map(
	...
	return await apiTryCatch(
		async () => {
			...
			if (filters?.status && Array.isArray(filters.status) && filters.status.length > 0) {
				const statusFilter = filters.status.map((s: string) => `status="${s}"`).join(' || ');
				filter += ` && (${statusFilter})`;
			} else {
				filter += ` && status!="archive"`;
			}
			...
			if (filters?.priority && Array.isArray(filters.priority) && filters.priority.length > 0) {
				const priorityFilter = filters.priority.map((p: string) => `priority="${p}"`).join(' || ');
		...
		'Failed to generate custom hierarchy',
		500
	);
};
