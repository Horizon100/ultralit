		...
		if (thread.project) {
			try {
				...
				const projectErrorMessage =
					projectErr instanceof Error ? projectErr.message : 'Unknown project error';
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		const isCollaborator = thread.project
			? await isProjectCollaborator(thread.project, userId)
			: false;
		const isMember = Array.isArray(thread.members) && thread.members.includes(userId);
		...
		if (
			!isOwner &&
			Object.keys(data).some((key) => !['members', 'updated', 'read_by'].includes(key))
		) {
