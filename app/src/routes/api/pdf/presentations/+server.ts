...
async function extractPageText(filePath: string, pageNumber: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const pdftotext = spawn('pdftotext', ['-f', pageNumber.toString(), '-l', pageNumber.toString(), filePath, '-']);
    let output = '';
...
function analyzeSlideStructure(slides: any[]): any {
  ...
  slides.forEach(slide => {
    ...
    } else if (textLower.includes('summary') || textLower.includes('conclusion') || 
               textLower.includes('thank you') || textLower.includes('questions')) {
