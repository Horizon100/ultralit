...
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	try {
		...
		const [messagesResult, documentsResult, tasksResult, completedTasksResult, lastMessageResult] =
		...
		const messageCount =
			messagesResult.status === 'fulfilled' ? messagesResult.value.totalItems : 0;
		const documentCount =
			documentsResult.status === 'fulfilled' ? documentsResult.value.totalItems : 0;
		const totalTasks = tasksResult.status === 'fulfilled' ? tasksResult.value.totalItems : 0;
		const completedTasks =
			completedTasksResult.status === 'fulfilled' ? completedTasksResult.value.totalItems : 0;
		...
		const collaboratorCount = Array.isArray(project.collaborators)
