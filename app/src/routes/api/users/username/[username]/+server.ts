...
export const GET: RequestHandler = async ({ params }) => {
  ...
  try {
    ...
    console.log('Original posts details:', originalPosts.items.map(p => ({
      id: p.id,
    ...
    console.log('Reposted posts details:', repostedPosts.items.map(p => ({
      id: p.id,
      content: p.content.substring(0, 30),
      repostedBy: p.repostedBy,
      user: p.user
    ...
    console.log('All posts with flags:', allPosts.map(p => ({
      id: p.id,
      isRepost: p.isRepost,
      content: p.content.substring(0, 30)
    })));
