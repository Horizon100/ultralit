    ...
    <header class="post-header">
      ...
      {#if user}
        <div class="header-user">
          <img 
            src={user.avatar ? `${pocketbaseUrl}/api/files/users/${user.id}/${user.avatar}` : '/api/placeholder/32/32'} 
        ...
        <article class="main-post">
          <div class="post-author">
            <a href="/{user.username}" class="author-link">
              <img 
                src={user.avatar ? `${pocketbaseUrl}/api/files/users/${user.id}/${user.avatar}` : '/api/placeholder/48/48'} 
                alt="{user.name || user.username}'s avatar" 
                class="author-avatar" 
              />
          ...
          <div class="post-actions">
            ...
            <button class="action-button upvote" on:click={() => handlePostInteraction(new CustomEvent('interact', { detail: { postId: post.id, action: 'upvote' } }))}>
              <Heart size={20} />
